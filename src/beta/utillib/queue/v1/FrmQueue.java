...
import beta.utillib.queue.IQueue;
import beta.utillib.queue.QueueTask;
...
public class FrmQueue extends JDialog {
    ...
    private final ResizingArray<QueuePanel> _WORKINGON = new ResizingArray<QueuePanel>();
...
//                }
                ...
                synchronized(_WORKINGON) {
                    for(int X = 0; X < _WORKINGON.length(); X++) {
                        if(_WORKINGON.getAt(X).getTask().equals(task)) {
                            _WORKINGON.removeAt(X);
                            break;
                        }
                    }
                }
...
//                synchronized(_WORKINGON) {
//                    for(int X = 0; X < _WORKINGON.length(); X++) {
//                        final QueueTask TASK = _WORKINGON.getAt(X).getTask();
//                        final Percent PERCENT = TASK.getPercent();
//
//                        if(PERCENT.isDifferentPercent()) {
//                            _WORKINGON.getAt(X).setText(TASK.getName() + " - " + PERCENT.getPreviousPercent() + "%");
//                        }
//
//                        if(TASK.isDone() || TASK.isCanceling() || TASK.isCanceled()) {
//                            _WORKINGON.removeAt(X);
//                            X--;
//                        }
//                    }
        ...
        public boolean cancelQuestion(boolean pause) {
            if(_TASK == null || _TASK.isDone()) {
                ...
                if(!_TASK.isCanceling() && !_TASK.isCanceled()) {
                    if(pause) {_TASK.pause(true);}

                    if(!_TASK.isCanceling() && !_TASK.isCanceled()) {
                        if(MsgUtil.msgboxYesNo(this, "Cancel Process?", "Queue")) {
                            return true;
                        } else {
                            if(pause) {_TASK.pause(false);}
...
//            System.out.println("layoutContainer");
    ...
    private static class DownloadSimulationQueueTask extends QueueTask {
		private final String _NAME;
		
		public DownloadSimulationQueueTask(String name) {
			_NAME = name;
		}
